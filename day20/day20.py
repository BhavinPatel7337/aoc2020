...
def find_left(tid, orientation, flipped):
    left = (orientation - 1 + 2 * flipped) % 4
...
product = 1
for c in corners:
    if find_left(c, 0, 0) and find_below(c, 0, 0):
        start = (c, 0, False)
    product *= c
print(product)
...
for i in range(len(tiles)):
    ...
    if (left := find_left(current, orientation, flipped)):
        current, orientation, flipped = left
    else:
